...
Token *lookAhead;     // token xem truoc // nguyen tac phan tich de quy tren xuong LL1 la ta phai doc truoc mot tu to de biet xem di theo huong nhu the nao

void scan(void) {
  Token* tmp = currentToken; // luu tru la token hien tai
  currentToken = lookAhead;   // doc token tiep theo
  lookAhead = getValidToken();  // doc ki tu moi o chuong trinh nguoi da cho, them vao bo pttv
  free(tmp);                  // giai phong con tro tmp
}

// eat xu li ki hieu ket thuc (hinh tron trong KPL)
// dua tren so do cu phap KPL xay dung chuong trinh
//neu so do cu phap co dang ki hieu ket thuc la hinh tron
void eat(TokenType tokenType) {       // xu li 1 token (ki hieu ket thuc hinh tron)
  if (lookAhead->tokenType == tokenType) {
    printToken(lookAhead);      // in token do ra
    scan();                 // doc token tiep theo
  } else missingToken(tokenType, lookAhead->lineNo, lookAhead->colNo); // nguoc lai in ra loi
}

//duyet ky hieu khong ket thuc va mot so do(ki hieu ko ket thuc la 1 hinh vuong, ngoai le identifier, number hinh vuong nhung coi la 1 ki hieu ket thuc boi vi no phan tich tu bo phan tich tu vung )
// dung complieA()

//void compileA(){
//  assert("Parsing a Program ...."); // assert giong printf("cho biet rang dang phan tich A")
//  T(X1);                            // ham phan tich X1
//  T(X2);                            // ham phan tch X2
//  ...
//  T(Xn);                            // ham phan tich Xn (ham con)
//  assert("Program parsed!");        // printf("da phan tich xong A")
}
...
// 08) <Block4> ::= <SubDecls> <Block5>|<Blook5>
...
// 14) <TypeDecls> ::= e // ep tren on la cau lenh rong
...
void compileParams(void){
  //TODO
  switch(lookAhead->tokenType){
    case SB_LPAR: // dau mo ngoac trai
      eat(SB_LPAR); // dau dong ngoac phai
      compileParam(); // goi ham comileParam
      compileParams2(); // goi ham compileParam2
      eat(SB_RPAR); // doc ki tu dong ngoac phai
    break;
    case SB_COLON:  // dau hai cham
    case SB_SEMICOLON: // dau cham phay
      break;
    default:
      error(ERR_INVALIDPARAM, lookAhead->lineNo, lookAhead->colNo);
      break;
...
void compileParams2(void) {
  ...
  case SB_SEMICOLON:
      ...
      compileParams2();
      break;
  case SB_RPAR: // dau dong ngoac phai thi break, nhin ham tren
      break;
  default:
      error(ERR_INVALIDPARAM, lookAhead->lineNo, lookAhead->colNo); // neu khong co truong hop nao o tren thi in ra thong bao  loi
      break;
  }
}
...
void compileParam(void){
  ...
  switch (lookAhead->tokenType){
    ...
    case KW_VAR:
      ...
      break;
    default:
      error(ERR_INVAILDPARAM, lookAhead->lineNo, lookAhead->colNo);
      break;
  }
}
...
// 54) <Statement> ::= <ForSt>
// 55) <Statement> ::= E // ep tren on la cau lenh rong
void compileStatement(void) {
  ...
  case KW_FOR:
    compileForSt();
    break;
    // EmptySt needs to check FOLLOW tokens // FOLLOW cho nao z
  case SB_SEMICOLON:        // dau cham phay
    eat(SB_SEMICOLON);      // doc vao dau cham phay
    break;
  case KW_END:
  case KW_ELSE:             // ???
    break;
...
void compileGroupSt(void) {
  ...
  // TODO
  eat(KW_BEGIN);  // doc vao tu khoa BEGIN
  compileStatements();
  eat(KW_END);
...
// 62) ElseSt::= E // eptren on la cau lenh rong
...
// 66) <Arguments> ::= E // ep tren on la cau lenh rong
void compileArguments(void) {
  ...
  case SB_LPAR:
      ...
      eat(SB_RPAR);
      break;
  // Follow - same as call statement as statement: // check follow cho nao
  case SB_SEMICOLON:  // dau ;
  case KW_END:
  case KW_ELSE:
  // Follow - term2
  case SB_TIMES:      // dau *
...
// 68) <Arguments2>::= e // rong, check folow
